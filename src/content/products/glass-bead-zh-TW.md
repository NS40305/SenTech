---
locale: zh-TW
slug: glass-bead
family: Specialty
name: 玻璃珠型 NTC 熱敏電阻（GB 系列）
tempMin: -40
tempMax: 250
tolerance: "±1 / ±3 / ±5 %"
bValue: "提供全系列 R/T 曲線材料"
dissipation: "GBL 1.1–1.8；GBM 1.0–1.6；GBS 0.7–0.9 mW/°C 靜止空氣"
timeConstant: "GBL 10–17 秒；GBM 9–16 秒；GBS 3.5–6.5 秒（空氣中）"
image: glass-bead
pdfUrl: /pdfs/glass-bead.pdf
summary: 玻璃密封徑向引線型 NTC，耐高溫至 250 °C，具優異耐潮濕與耐化學特性，適用於需高穩定性的高溫感測。
---

GB 系列玻璃珠型 NTC 採熔融玻璃密封晶片，提供卓越的長期穩定性，並耐潮濕、溶劑與惡劣介質。GBL／GBM／GBS 三款子系列在尺寸、散逸常數與時間常數間取得不同平衡，可依反應速度與自熱裕度需求挑選。典型應用包含低成本探頭組件、高溫印表機墨水感測、PCB 溫度感測、空調系統，以及高溫高速輪胎製程。
